...
\033[1;32m.....
         ...
         WWWWWWWW,xWx WWO      ;WWWWWWWX          .WWl
         ,,lMM0,, ckc WM0 ,dd: .,,0MM;,..ckKKKkc. .MMo 'ddc
           'MMk   XMN WMXOMK;     kMM  'NMK:,:KMN..MM00M0,
           'MMk   XMN WMMXMK.     kMM  cMMc   cMM:.MMWXM0.
           'MMk   XMN WM0 dMW:    kMM  .OMWkokMMO..MMo oMWc
           .xx:   oxd xxl  ,xx,   :xx    'lxxdl'  .xx,  ;xx'
\033[1;34m╠═════════════════════════════════════════════════════════════════╣
\033[1;32m║➢ Author   : DIEN BASE64                                         ║
\033[1;36m║➢ TOOL   :    100% BUFF VIEW TIKTOK                              ║
\033[1;31m║➣  Zalo  :   0899717143                                          ║
\033[1;34m╚═════════════════════════════════════════════════════════════════╝"""
        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(f'{do}[{vang}Dien - base{do}] {do}[{xanh_la}BUFF attack tiktok{do}] {do}{xanh_duong}+: {trang}{success}{do} View+plus')
                    success += 1
                    _lock.release()
            except:
...
if __name__ == "__main__":
    ...
    print(banner)
    try:
        link = input(f'{do}[{trang}{kt_code}{do}] {xanh_la}nhap link tiktok vao: {trang}')
        print(f'{trang}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -')
        __aweme_id = str(
        ...
        exit(f"{do}[{trang}{kt_code}{do}] {do}sai con heo no roi")
